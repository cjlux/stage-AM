...
class xsens_mti_listener_angular_velocity:
    ...
    def __init__(self, opened_log_file, verbose=False):
        '''Parameters :
	   	...
	   	subscriber : register messages that came from topic "/imu/angular_velocity",
		             vector3Stamped is the type of data received,
			     callback() calls data when it is received.
        '''
        self.verbose = verbose
        self.log_file = opened_log_file
        self.subscriber = rospy.Subscriber("/imu/angular_velocity", Vector3Stamped, self.callback)
	...
	'''
        ...
        fb_data = data.vector.split(":")                        # Retrieve the angular velocities in x, y and z
